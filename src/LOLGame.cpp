...
void LOLGame::Player::PrintPlayer() {
    cout << "LVL " << summonerLevel << " Summoner" << endl;
...
void LOLGame::ReadGameDataFromFile(istringstream &stream) {
    ...
    gameDuration
    gameID
    for participants 0-9 in order: assists, championLevel, championName, deaths, goldEarned, goldSpent, itemsPurchased, kills, summonerLevel, totalDamageDealtToChampions
    ...
    */
    ...
    for (int i = 0; i < 10; i++) {
        ...
        getline(stream, token);
        int tddc = stoi(token);
        players.push_back(new Player(a, cl, cn, d, ge, gs, ip, k, sl, tddc));
    }

    // Team 0 (Blue) stats
    getline(stream, token);
    team0Win = token == "True" ? true : false;
    getline(stream, token);
    ...
    getline(stream, token);
    getline(stream, token);
    team1Win = token == "True" ? true : false;
    getline(stream, token);
